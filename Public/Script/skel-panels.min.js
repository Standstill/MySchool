...
jQuery.fn.clearTransition_skel=function(){return jQuery(this).xcss_skel("transition","none")};
jQuery.fn.resetForms_skel=function(){var a=jQuery(this);jQuery(this).find("form").each(function(){this.reset()});return a};
	jQuery.fn.initialize_skel=function(){
		var a=jQuery(this);
		a.attr("class").match(/(\s+|^)([0-9]+)u(\s+|$)/)&&a.data("cell-size",parseInt(RegExp.$2))
	};
	jQuery.fn.expandCell_skel=function(){
		var a=jQuery(this),b=12;
		a.parent().children().each(function(){
			var a=$(this).attr("class");
			a&&a.match(/(\s+|^)([0-9]+)u(\s+|$)/)&&(b-=parseInt(RegExp.$2))});
		0<b&&(a.initialize_skel(),a.css("width",100*((a.data("cell-size")+b)/12)+"%"))
	}
},
initObjects:function(){
	a.cache.window=jQuery(window);
	a.cache.window.load(function(){0==a.cache.window.scrollTop()&&window.scrollTo(0,1)});
	a._.DOMReady(function(){
		a.cache.body=jQuery("body");
		a.cache.body.wrapInner('<div id="skel-panels-pageWrapper" />');
		a.cache.pageWrapper=jQuery("#skel-panels-pageWrapper");
		a.cache.pageWrapper
			.css("position","relative").css("left","0").css("right","0").css("top","0").css("bottom","0")
			.applyTransition_skel().accelerate_skel();
		a.cache.defaultWrapper=jQuery('<div id="skel-panels-defaultWrapper" />').appendTo(a.cache.body);
		a.cache.defaultWrapper.css("height","auto");
		a.cache.fixedWrapper=jQuery('<div id="skel-panels-fixedWrapper" />').appendTo(a.cache.body);
		a.cache.fixedWrapper.css("position","relative");
		jQuery(".skel-panels-fixed").appendTo(a.cache.fixedWrapper);
		a._.registerLocation("skel_panels_defaultWrapper",a.cache.defaultWrapper[0]);
		a._.registerLocation("skel_panels_fixedWrapper",a.cache.fixedWrapper[0]);
		a._.registerLocation("skel_panels_pageWrapper",a.cache.pageWrapper[0])
	})
},
initDeviceType:function(){var d,b={ios:"(iPad|iPhone|iPod)",android:"Android"};for(d in b)if(navigator.userAgent.match(RegExp(b[d],"g"))){a.deviceType=d;break}a.deviceType||(a.deviceType="other");a.isTouch=!!("ontouchstart"in window);a.eventType=a.isTouch?"touchend":"click"},init:function(){a.initDeviceType();a.initHelpers();a.initObjects();a.initElements("overlay");a.initElements("panel");a._.updateState()}};return a}());